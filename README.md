...
Detecting solid shapes on textured backgrounds, tracing their outlines,
marking their centers, and estimating their 3D positions from the camera
intrinsics: on a static image, a video, and a video where the background
changes completely.
...
| Part 4 – 3D coordinates | drawn as `(x, y, z) in` labels in all outputs |
...
(GitHub file size limits), so drop the three challenge files into `media/`.
...
2x downscaled frame and scaling the contours back up cut it to ~7ms with no
...
Given `K = [[2564.3, 0, 0], [0, 2569.7, 0], [0, 0, 1]]` and the circle's real
radius (10 in):

- The circle is the only shape with known physical size, so it anchors depth:
  `Z = fx * R / r_px` where `r_px` is its pixel radius.
- Flat-surface assumption -> every shape shares that Z.
- Back-project each center: `X = (u - cx)*Z/fx`, `Y = (v - cy)*Z/fy`.

Since cx/cy in the given K are 0, I read pixel coordinates relative to the
image center (i.e. treated the principal point as the center of the frame).
fx/fy are scaled when the frame isn't 1920 wide (the static png is 960x540).

**Challenges:** two failure modes, both about picking the wrong depth anchor.
(1) When the circle is clipped by the frame edge its apparent radius shrinks
and Z inflates, so border-touching blobs are rejected as anchors and the last
good Z is carried forward (the ground plane doesn't move between frames).
(2) My first anchor filter was "any blob with circularity > 0.8", and the
pentagon (circularity ~ 0.86) kept winning whenever the real circle was off
screen, which read the pentagon's bigger radius as the circle's and knocked
~15% off the depth. Anchoring only on the shape *classified* as a circle fixed
it: Z now sits at ~261 in across the grass video and ~263–264 in on the hard
video, consistent with a fixed camera height.

## Repo layout

```
detect.py     the whole algorithm (texture mask, contours, overlap split, 3D)
